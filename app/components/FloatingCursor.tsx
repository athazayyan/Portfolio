...
const FloatingCursors = () => {
  ...
  const newElements = Array.from({ length: total }).map((_, i) => {
    ...
    return {
      ...
      ...testimonial,
      startX,
      startY: startY + 40,     // mulai dari bawah
      endX: startX + (Math.random() * 20 - 10),
      endY: startY - 30,       // bergerak ke atas
  ...
  return (
    <div className="absolute inset-0 z-5 overflow-visible pointer-events-auto">
      {elements.map((el) => (
        <motion.div
          key={el.id}
          className="absolute flex items-start gap-2 group cursor-pointer"
